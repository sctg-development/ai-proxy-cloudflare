 ...
 * @returns The conversations endpoint URL as a string.
 */
...
/**
 * Options for building a Mistral conversations request payload.
 */
export interface MistralConversationsPayloadOptions {
  /** The model ID to send the request to. */
  modelId: string;
  /** System prompt passed as the `instructions` field. */
  systemPrompt: string;
  /** The conversation messages to convert to the inputs array. */
...
/**
 * Extracts plain text from a single playground part for inclusion in the
 * Mistral conversations `inputs` array.
 *
 * Returns `null` for parts that cannot be represented as text (e.g. inline images,
 * generic files without `textContent`).
 *
 * @param part - A single playground part to convert.
 * @returns The text representation, or `null` if the part has no text equivalent.
 */
...
/** Generic JSON object used to safely inspect the Mistral conversations response. */
 ...
 *
 * @param body - The raw response body from the Mistral conversations endpoint.
 * @returns An array of playground parts (text and image parts).
 */
